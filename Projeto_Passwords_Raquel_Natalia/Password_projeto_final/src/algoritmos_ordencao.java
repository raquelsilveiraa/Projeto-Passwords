public class algoritmos_ordencao {
    ...
    public static void bubbleSort(Dados_password[] array) {
        ...
        // countingSort
        public static Dados_password[] countingSort(Dados_password[] array) {
            int max = 0;
            for (Dados_password aux : array) {
                if (aux.getLength() > max) {
                    max = aux.getLength();
                }
            }
            ...
            for (int i = 0; i < array.length; i++) {
                Dados_password aux = array[i];
                int j = contador[aux.getLength()] - 1;
                saida[j] = aux; 
                contador[aux.getLength()]--;
            }
        
            for (int i = 0; i < saida.length / 2; i++) {
                Dados_password aux_02 = saida[i];
                saida[i] = saida[saida.length - i - 1];
                saida[saida.length - i - 1] = aux_02;
            }
